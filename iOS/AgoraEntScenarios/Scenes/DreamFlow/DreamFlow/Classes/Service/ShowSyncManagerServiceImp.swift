...
private func agoraPrint(_ message: String) {
    DreamFlowLogger.info(message, context: "Service")
}
...
public class ShowSyncManagerServiceImp: NSObject {
    ...
    required init(appId: String, host: String, userInfo: DreamFlowUserInfo) {
        self.appId = appId
        self.host = host
        let owner = AUIUserThumbnailInfo()
        owner.userId = userInfo.uid
        owner.userName = userInfo.userName
        owner.userAvatar = userInfo.avatar
        self.user = owner
        AUIRoomContext.shared.displayLogClosure = { msg in
            DreamFlowLogger.info(msg, context: "RTMSyncManager")
        }
        super.init()
        syncManager.rtmManager.subscribeError(channelName: "", delegate: self)
        
        DreamFlowContext.shared.agoraRTCToken = ""
        DreamFlowContext.shared.agoraRTMToken = ""
    }
...
extension ShowSyncManagerServiceImp {
    ...
    private func preGenerateToken(completion:@escaping (NSError?)->()) {
        ...
        NetworkManager.shared.generateToken(channelName: "",
            ...
            DreamFlowContext.shared.agoraRTCToken = rtcToken
            DreamFlowContext.shared.agoraRTMToken = rtmToken
            completion(nil)
        }
    }
    
    private func login(completion:(@escaping (NSError?)-> Void)) {
        let token = DreamFlowContext.shared.agoraRTMToken
        if !token.isEmpty {
            let date = Date()
