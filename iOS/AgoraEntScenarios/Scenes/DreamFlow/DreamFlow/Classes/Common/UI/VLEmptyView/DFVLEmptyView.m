...
@interface DFVLEmptyView ()

@property (nonatomic, weak) id <DFVLEmptyViewDelegate>delegate;
@property (nonatomic, strong) UIImageView *showImageView;
@property (nonatomic, strong) UILabel *textLabel;
@property (nonatomic, strong) UIButton *button;
@end

@implementation DFVLEmptyView
...
- (void)makeConstraintsSubViews {
    ...
    [self.showImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self).offset(DL_VLREALVALUE_WIDTH(84));
        make.left.equalTo(self).offset(DL_VLREALVALUE_WIDTH(97));
        make.width.mas_offset(DL_VLREALVALUE_WIDTH(180));
        make.height.mas_offset(DL_VLREALVALUE_WIDTH(180));
    }];
    
    [self addSubview:self.textLabel];
    [self.textLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.showImageView.mas_bottom).offset(DL_VLREALVALUE_WIDTH(24));
        make.left.right.equalTo(self);
        make.height.mas_offset(DL_VLREALVALUE_WIDTH(21));
    }];
    
    [self addSubview:self.detailTextLabel];
    [self.detailTextLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.textLabel.mas_bottom).offset(DL_VLREALVALUE_WIDTH(10));
        make.left.right.equalTo(self);
        make.height.mas_offset(DL_VLREALVALUE_WIDTH(17));
    }];
    
    [self addSubview:self.button];
    [self.button mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.detailTextLabel.mas_bottom).offset(DL_VLREALVALUE_WIDTH(118));
        make.centerX.equalTo(self.mas_centerX);
        make.width.mas_offset(DL_VLREALVALUE_WIDTH(92));
        make.height.mas_offset(DL_VLREALVALUE_WIDTH(45));
    }];
}
#pragma mark - Events
...
- (UIButton *)button {
    if (!_button) {
//        _button = [[QMUIButton alloc] qmui_initWithImage:nil title:AGLocalizedString(@"back")];
        ...
        _button.titleLabel.font = DFUIFontMake(12);
        _button.layer.cornerRadius = DL_VLREALVALUE_WIDTH(45)/2.0;
    }
    return _button;
}
...
- (UILabel *)textLabel{
    if (!_textLabel) {
        ...
        _textLabel.font = DFUIFontMake(15);
...
- (UILabel *)detailTextLabel{
    if (!_detailTextLabel) {
        ...
        _detailTextLabel.font = DFUIFontMake(12);
