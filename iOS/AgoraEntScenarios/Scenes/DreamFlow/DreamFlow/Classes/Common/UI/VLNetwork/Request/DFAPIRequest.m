...
#import "DFAPIRequest.h"
//#import "AppDelegate+Config.h"
#import "DFMacroDefine.h"
#import "DFToast.h"
#import "DFUserCenter.h"
#import "NSString+DFHelper.h"
#import "NSObject+DFAppInfo.h"
@import AFNetworking;
@import YYModel;
...
+ (void)getRequestURL:(NSString *)url parameter:(id)parameter showHUD:(BOOL)show success:(completeBlock_success)success failure:(errorBlock_fail)failure {
    [self requestRoute:@"" showHUD:show method:url parameter:parameter requestType:DFRequestTypeGet progressBlock:nil completeBlock:success errorBlock:failure];
}
...
+ (void)postRequestURL:(NSString *)url parameter:(id)parameter showHUD:(BOOL)show success:(completeBlock_success)success failure:(errorBlock_fail)failure {
    [self requestRoute:@"" showHUD:show method:url parameter:parameter requestType:DFRequestTypePost progressBlock:nil completeBlock:success errorBlock:failure];
}


+ (void)requestRoute:(NSString *)route showHUD:(BOOL)show method:(NSString *)method parameter:(id)json requestType:(DFRequestType)type progressBlock:(progressBlock)progressBlock completeBlock:(completeBlock_success)completeBlock errorBlock:(errorBlock_fail)errorBlock {
    ...
    if (type == DFRequestTypeGet) {
        ...
        sessionTask = [_sessionManager GET:url parameters:paramenter headers:@{} progress:^(NSProgress * _Nonnull downloadProgress) {
            ...
            DFResponseDataModel *model = [DFResponseDataModel yy_modelWithDictionary:dic];
            ...
            DFLog(@"\nncomplete:\n%@\nheader:\n%@\nparams:\n%@\nResponse:%@",task.currentRequest.URL,_sessionManager.requestSerializer.HTTPRequestHeaders,paramenter,error);
    ...
    } else if (type == DFRequestTypePost) {
        NSDictionary *paramenter = [self setCommonParamenter:json];
...
+ (void)requestImageRoute:(NSString *)route  method:(NSString *)method parameter:(id)json requestType:(DFRequestType)type progressBlock:(progressBlock)progressBlock completeBlock:(completeImageBlock_success)completeBlock errorBlock:(errorBlock_fail)errorBlock {
...
+ (NSDictionary *)dicWithResponseData:(NSData *)res {
    ...
    if (err) {
        DFLog(@"responseData transform dictionary error!");
    }
    return dic;
}
...
+ (__kindof NSURLSessionDataTask *)uploadImagesRoute:(NSString *)route
    ...
    NSURLSessionDataTask *sessionTask = [_sessionManager POST:url parameters:paramenter headers:@{} constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        if (show) [SVProgressHUD dismiss];
        NSString *resultStr = [[NSString alloc] initWithData:responseObject encoding:NSUTF8StringEncoding];
        DFResponseDataModel *model = [DFResponseDataModel yy_modelWithJSON:resultStr];
        DFLog(@"\ncomplete:\n%@\nheader:\n%@\nparams:\n%@\nResponse:\n%@\nModel:%@",task.currentRequest.URL,_sessionManager.requestSerializer.HTTPRequestHeaders,paramenter,resultStr,[model yy_modelDescription]);
        [self requestSuccess:completeBlock object:responseObject method:method task:task];
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        if (show) [SVProgressHUD dismiss];
        DFLog(@"\nncomplete:\n%@\nheader:\n%@\nparams:\n%@\nResponse:%@",task.currentRequest.URL,_sessionManager.requestSerializer.HTTPRequestHeaders,paramenter,error);
...
+ (void)setLoginVC {
    [DFToast toast:AGLocalizedString(@"app_expire")];
    // TODO: goto login page
    [[DFUserCenter center] logout];
...
+ (void)requestSuccess:(completeBlock_success)completeBlock object:(id)object method:(NSString *)method task:(NSURLSessionDataTask *)task{
    ...
    DFResponseDataModel *model = [DFResponseDataModel yy_modelWithJSON:resultStr];
    if (!model) {
        model = [[DFResponseDataModel alloc]init];
