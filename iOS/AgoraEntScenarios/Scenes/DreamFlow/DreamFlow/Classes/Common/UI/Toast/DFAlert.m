...
#import "DFAlert.h"
#import "DFAttributedTextView.h"

@interface DFAlert()<UITextFieldDelegate, UITextViewDelegate>
...
@property (nonatomic, strong) DFAttributedTextView *attrView;
...
@implementation DFAlert

static DFAlert *_alert = nil;
+ (instancetype)shared
{
    if (!_alert) {
        _alert = [[DFAlert alloc] init];
    }
    return _alert;
}
...
-(void)showAttributeAlertWithFrame:(CGRect)frame title:(NSString * _Nullable)title text:(NSString *)text AttributedStringS:(NSArray *)strings ranges:(NSArray *)ranges textColor:(UIColor *)textColor attributeTextColor:(UIColor * )attributeTextColor buttonTitles:(NSArray *)buttonTitles completion:(OnCallback _Nullable)completion linkCompletion:(linkCallback _Nullable)linkCompletion{
        ...
        self.attrView = [[DFAttributedTextView alloc]initWithFrame:CGRectZero text:text AttributedStringS:strings ranges:ranges textColor:textColor attributeTextColor:attributeTextColor];
