...
@interface DFQMUIHelper ()

@end

@implementation DFQMUIHelper (Device)
...
+ (BOOL)is67InchScreen {
    if (is67InchScreen < 0) {
        is67InchScreen = (DF_DEVICE_WIDTH == self.screenSizeFor67Inch.width && DF_DEVICE_HEIGHT == self.screenSizeFor67Inch.height) ? 1 : 0;
    }
...
+ (BOOL)is65InchScreen {
    if (is65InchScreen < 0) {
        ...
        is65InchScreen = (DF_DEVICE_WIDTH == self.screenSizeFor65Inch.width && DF_DEVICE_HEIGHT == self.screenSizeFor65Inch.height && !DFQMUIHelper.is61InchScreen) ? 1 : 0;
    }
...
+ (BOOL)is61InchScreenAndiPhone12Later {
    if (is61InchScreenAndiPhone12Later < 0) {
        is61InchScreenAndiPhone12Later = (DF_DEVICE_WIDTH == self.screenSizeFor61InchAndiPhone12Later.width && DF_DEVICE_HEIGHT == self.screenSizeFor61InchAndiPhone12Later.height) ? 1 : 0;
    }
...
+ (BOOL)is61InchScreen {
    if (is61InchScreen < 0) {
        is61InchScreen = (DF_DEVICE_WIDTH == self.screenSizeFor61Inch.width && DF_DEVICE_HEIGHT == self.screenSizeFor61Inch.height && ([[DFQMUIHelper deviceModel] isEqualToString:@"iPhone11,8"] || [[DFQMUIHelper deviceModel] isEqualToString:@"iPhone12,1"])) ? 1 : 0;
    }
...
+ (BOOL)is54InchScreen {
    if (is54InchScreen < 0) {
        is54InchScreen = (DF_DEVICE_WIDTH == self.screenSizeFor54Inch.width && DF_DEVICE_HEIGHT == self.screenSizeFor54Inch.height) ? 1 : 0;
    }
...
+ (CGFloat)preferredLayoutAsSimilarScreenWidthForIPad {
    if (preferredLayoutWidth < 0) {
        ...
        preferredLayoutWidth = DF_SCREEN_WIDTH;
...
+ (UIEdgeInsets)safeAreaInsetsForDeviceWithNotch {
    ...
    NSString *deviceKey = [DFQMUIHelper deviceModel];
    if (!dict[deviceKey]) {
