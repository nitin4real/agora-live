...
extension UIImage {
    ...
    @objc static func sceneImage(name: String, bundleName: String?) -> UIImage? {
        let bundleName = DreamFlowContext.sceneLocalizeBundleName
        
        let cacheName = "\(name)__\(bundleName)"
        if let image = DreamFlowContext.shared.imageCahe[cacheName] as? UIImage {
            return image
        }
        ...
        let lang = DreamFlowContext.shared.getLang()
        for value in scales {
            ...
            if let path = bundle.path(forResource: imageName2, ofType: suffix) {
                let image = UIImage(contentsOfFile: path)
                assert(image != nil, "image == nil \(path)")
                DreamFlowContext.shared.imageCahe[cacheName] = image
                return image
            }
        }
