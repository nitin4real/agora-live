...
class NetworkManager:NSObject {
    ...
    private var sessionConfig: URLSessionConfiguration = {
        ...
        if !DFUserCenter.user.token.isEmpty {
            config.httpAdditionalHeaders?["Authorization"] = DFUserCenter.user.token
        }
    ...
    private var baseServerUrl: String {
        return DreamFlowContext.releaseBaseServerUrl ?? ""
    }
    ...
    public func generateToken(channelName: String,
                              ...
                              expire: UInt = 24 * 60 * 60,
                              success: @escaping (String?) -> Void) {
        let params = ["appCertificate": DreamFlowContext.appCertificate,
                      "appId": DreamFlowContext.appId,
...
//            ToastView.hidden()
    ...
    func generateIMConfig(type: Int,
        ...
        let imConfig = [
            "appKey":DreamFlowContext.imAppKey,
            "clientId":DreamFlowContext.imClientId,
            "clientSecret":DreamFlowContext.imClientSecret,
        ]
...
//            ToastView.hidden()
    ...
    func startCloudPlayer(channelName: String,
                          ...
                          streamUrl: String,
                          success: @escaping (String?) -> Void) {
        let params: [String: Any] = ["appCertificate": DreamFlowContext.appCertificate,
                                     "appId": DreamFlowContext.appId,
                                     "channelName": channelName,
                                     "uid": uid,
                                     "robotUid": robotUid,
    ...
    func cloudPlayerHeartbeat(channelName: String,
                              uid: String,
                              success: @escaping (String?) -> Void) {
        let params: [String: String] = ["appCertificate": DreamFlowContext.appCertificate,
                                        "appId": DreamFlowContext.appId,
                                        "channelName": channelName,
                                        "uid": uid,
                                        "src": "iOS",
...
extension NetworkManager {
    ...
    public func reportDeviceInfo(sceneName: String) {
        ...
        let url = DreamFlowContext.host + "/api-login/report/device?userNo=\(DFUserCenter.user.userNo)&sceneId=\(sceneName)&appId=\(DreamFlowContext.appId)&projectId=agora_ent_demo"
        NetworkManager.shared.postRequest(urlString: url,
                                          params: params,
                                          success: { response in
            print(response)
    ...
    public func reportUserBehavior(sceneName: String) {
        ...
        let url = DreamFlowContext.host + "/api-login/report/action?userNo=\(DFUserCenter.user.userNo)&sceneId=\(sceneName)&appId=\(DreamFlowContext.appId)&projectId=agora_ent_demo"
        NetworkManager.shared.postRequest(urlString: url,
                                          params: params,
                                          success: { response in
            print(response)
