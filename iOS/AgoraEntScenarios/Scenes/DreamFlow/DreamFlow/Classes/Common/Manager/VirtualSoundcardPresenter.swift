...
@objc class VirtualSoundcardPresenter: NSObject {
    ...
    private func applyParams() {
        let paramStr = String(format: "{\"che.audio.virtual_soundcard\":{\"preset\":%ld,\"gain\":%ld,\"gender\":%d,\"effect\":%d}}", self.presetValue, self.gainValue, self.gender, self.effect)
        DreamFlowLogger.info("virtual sound card applyParams: \(paramStr)")
        self.rtcKit?.setParameters(paramStr)
    }
