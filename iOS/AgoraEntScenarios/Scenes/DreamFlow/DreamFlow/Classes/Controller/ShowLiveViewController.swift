...
//            loadingType = .prejoined
    ...
    var loadingType: AnchorState = .prejoined {
        didSet {
            ...
            case .joinedWithVideo, .joinedWithAudioVideo:
                if let room = room {
                    serviceImp = DreamFlowContext.showServiceImp()
                    _joinRoom(room)
                }
            }
        }
    ...
    private var currentUserId: String {
        get{
            DFUserCenter.user.id
        }
    }
    
    private var isPublishCameraStream: Bool {
        return role == .broadcaster || currentInteraction?.userId == DFUserCenter.user.id
    }

    private var role: AgoraClientRole {
        return room?.ownerId == DFUserCenter.user.id ? .broadcaster : .audience
    }
    ...
    private var interactionList: [ShowInteractionInfo]? {
        didSet {
            if DreamFlowContext.shared.rtcToken?.isEmpty == false {
    ...
    private var currentInteraction: ShowInteractionInfo? {
        didSet {
            if let currentInteraction = currentInteraction {
                DreamFlowLogger.info("currentInteraction: \(currentInteraction.description)")
            }
    ...
    private var muteLocalAudio: Bool = false {
        didSet {
            DreamFlowLogger.info("muteLocalAudio: \(muteLocalVideo)")
    ...
    private var muteLocalVideo: Bool = false {
        didSet {
            DreamFlowLogger.info("muteLocalVideo: \(muteLocalVideo)")
    ...
    deinit {
        DreamFlowLogger.info("deinit-- ShowLiveViewController \(self)")
    }
    
    override init(nibName nibNameOrNil: String?, bundle nibBundleOrNil: Bundle?) {
        super.init(nibName: nibNameOrNil, bundle: nibBundleOrNil)
        DreamFlowLogger.info("init-- ShowLiveViewController \(self)")
    }
    ...
    override func viewDidLoad() {
        ...
        setupUI()
        if room.ownerId == DFUserCenter.user.id {
            self.joinChannel()
    ...
    func leaveRoom(){
        if #available(iOS 13.0, *) {
            AgoraEntLog.autoUploadLog(scene: DreamFlowLogger.kLogKey)
        } else {
    ...
    private func sendMessageWithText(_ text: String) {
        ...
        showMsg.userId = DFUserCenter.user.id
        showMsg.userName = DFUserCenter.user.name
...
extension ShowLiveViewController {
    ...
    func _joinRoom(_ room: ShowRoomListModel){
        ...
        if let service = serviceImp {
            ...
            DreamFlowLogger.info("serviceImp is nil, roomid = \(roomId)")
...
//            }
    ...
    private func _broadcasterRoomExpired(){
        ...
        ownerExpiredView?.headImg = DFUserCenter.user.headUrl
...
//            ToastView.show(text: "net work error: \(state)")
    ...
    func onUserLeftRoom(channelName: String, user: ShowUser) {
        if user.userId == room?.ownerId {
            DreamFlowLogger.info(" finishAlertVC onUserLeftRoom : roomid = \(roomId)")
            onRoomExpired(channelName: channelName)
        }
    }
    ...
    func onMicSeatApplyUpdated(channelName: String, applies: [ShowMicSeatApply]) {
        DreamFlowLogger.info("onMicSeatApplyUpdated: \(applies.count)")
        _updateApplyMenu()
        if role == .broadcaster {
    ...
    func onMicSeatInvitationUpdated(channelName: String, invitation: ShowMicSeatInvitation) {
        guard invitation.userId == DFUserCenter.user.id, inviteVC == nil else { return }
    ...
    private func _onStartInteraction(interaction: ShowInteractionInfo) {
        DreamFlowLogger.info("_onStartInteraction: \(interaction.userId) \(interaction.userName) status: \(interaction.type.rawValue)")
    ...
    private func _onStopInteraction(interaction: ShowInteractionInfo) {
        DreamFlowLogger.info("_onStopInteraction: \(interaction.userId) \(interaction.userName) status: \(interaction.type.rawValue)")
        switch interaction.type {
        case .pk:
            view.layer.contents = UIImage.show_sceneImage(name: "show_live_room_bg")?.cgImage
...
//            currentInteraction?.ownerMuteAudio = false
            ...
            let toRole: AgoraClientRole? = interaction.userId == DFUserCenter.user.id ? .audience : nil
            ShowAgoraKitManager.shared.switchRole(role: toRole,
                                                  channelId: roomId,
                                                  options: self.channelOptions,
                                                  uid: interaction.userId)
...
extension ShowLiveViewController: AgoraRtcEngineDelegate {
    ...
    func rtcEngine(_ engine: AgoraRtcEngineKit, didOccurWarning warningCode: AgoraWarningCode) {
        DreamFlowLogger.warn("rtcEngine warningCode == \(warningCode.rawValue)")
    }
    func rtcEngine(_ engine: AgoraRtcEngineKit, didOccurError errorCode: AgoraErrorCode) {
        DreamFlowLogger.warn("rtcEngine errorCode == \(errorCode.rawValue)")
    }
    
    func rtcEngine(_ engine: AgoraRtcEngineKit, didJoinChannel channel: String, withUid uid: UInt, elapsed: Int) {
        DreamFlowLogger.info("rtcEngine didJoinChannel \(channel) with uid \(uid) elapsed \(elapsed)ms")
    }
    
    func rtcEngine(_ engine: AgoraRtcEngineKit, didJoinedOfUid uid: UInt, elapsed: Int) {
        DreamFlowLogger.info("rtcEngine didJoinedOfUid \(uid) channelId: \(roomId)", context: kDreamFlowLogBaseContext)
    }

    func rtcEngine(_ engine: AgoraRtcEngineKit, didOfflineOfUid uid: UInt, reason: AgoraUserOfflineReason) {
        DreamFlowLogger.info("rtcEngine didOfflineOfUid === \(uid) reason: \(reason.rawValue)")
    }
    ...
    public func rtcEngine(_ engine: AgoraRtcEngineKit, didVideoMuted muted: Bool, byUid uid: UInt) {
        DreamFlowLogger.info("didVideoMuted[\(uid)] \(muted)")
    ...
    func rtcEngine(_ engine: AgoraRtcEngineKit, didAudioMuted muted: Bool, byUid uid: UInt) {
        DreamFlowLogger.info("didAudioMuted[\(uid)] \(muted)")
        if "\(uid)" == currentInteraction?.userId {
    ...
    func rtcEngine(_ engine: AgoraRtcEngineKit, contentInspectResult result: AgoraContentInspectResult) {
        DreamFlowLogger.warn("contentInspectResult: \(result.rawValue)")
    ...
    func rtcEngine(_ engine: AgoraRtcEngineKit, firstLocalVideoFramePublishedWithElapsed elapsed: Int, sourceType: AgoraVideoSourceType) {
        DreamFlowLogger.info("firstLocalVideoFramePublishedWithElapsed: \(elapsed)ms \(sourceType.rawValue)",
                        context: kDreamFlowLogBaseContext)
    }
    
    func rtcEngine(_ engine: AgoraRtcEngineKit, tokenPrivilegeWillExpire token: String) {
        DreamFlowLogger.warn("tokenPrivilegeWillExpire: \(roomId)",
                        context: kDreamFlowLogBaseContext)
...
extension ShowLiveViewController: ShowRoomLiveViewDelegate {
    ...
    func onClickRemoteCanvas() {
        ...
        toolView.heightAnchor.constraint(equalToConstant: 150 + Screen.DFSafeAreaBottomHeight()).isActive = true
    ...
    func onClickSettingButton() {
        ...
        if interactionStatus == .idle {
            ...
            settingMenuVC.type = role == .broadcaster ? .pking : (currentInteraction?.userId == DFUserCenter.user.id ? .pking : .idle_audience)
