...
class ShowRoomListVC: UIViewController {
    ...
    private var dfUserInfo: DreamFlowUserInfo!
    ...
    private lazy var delegateHandler = {
        let handler = ShowCollectionLoadingDelegateHandler(localUid: dfUserInfo.getUIntUserId())
...
//        }
    ...
    required init(userInfo: DreamFlowUserInfo) {
        self.dfUserInfo = userInfo
    ...
    deinit {
        DreamFlowContext.unloadShowServiceImp()
        ShowAgoraKitManager.shared.destoryEngine()
        DreamFlowLogger.info("deinit-- ShowRoomListVC")
    }
    
    override init(nibName nibNameOrNil: String?, bundle nibBundleOrNil: Bundle?) {
        super.init(nibName: nibNameOrNil, bundle: nibBundleOrNil)
        DreamFlowContext.sceneLocalizeBundleName = "showResource"
        DreamFlowLogger.info("init-- ShowRoomListVC")
        VideoLoaderApiImpl.shared.printClosure = { msg in
            DreamFlowLogger.info(msg, context: "VideoLoaderApi")
        }
        VideoLoaderApiImpl.shared.warningClosure = { msg in
            DreamFlowLogger.warn(msg, context: "VideoLoaderApi")
        }
        VideoLoaderApiImpl.shared.errorClosure = { msg in
            DreamFlowLogger.error(msg, context: "VideoLoaderApi")
        }
    }
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        _ = stateManager
        DreamFlowContext.shared.sceneImageBundleName = "showResource"
        createViews()
        createConstrains()
    ...
    private func joinRoom(_ room: ShowRoomListModel){
        ...
        if room.ownerId == DFUserCenter.user.id {
            ...
            vc.roomList = roomList.filter({ $0.ownerId != DFUserCenter.user.id })
    ...
    private func fetchRoomList() {
        DreamFlowContext.showServiceImp()?.getRoomList(page: 1) { [weak self] error, roomList in
            guard let self = self else { return }
            self.collectionView.mj_header?.endRefreshing()
            if let error = error {
                DreamFlowLogger.error(error.localizedDescription)
                return
            }
            let list = roomList ?? []
            let dislikeRooms = DreamFlowContext.shared.dislikeRooms()
            let dislikeUsers = DreamFlowContext.shared.dislikeUsers()
    ...
    private func checkTokenValid() {
        if DreamFlowContext.shared.rtcToken?.count ?? 0 > 0, let date = DreamFlowContext.shared.tokenDate, Int64(-date.timeIntervalSinceNow) < 20 * 60 * 60 {
            return
        }
        preGenerateToken()
    }
...
extension ShowRoomListVC: UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.ag_addPreloadTap(roomInfo: room, localUid: delegateHandler.localUid) {[weak self] state in
            if DreamFlowContext.shared.rtcToken?.count ?? 0 == 0 {
                if state == .began {
...
extension ShowRoomListVC {
    ...
    func createConstrains() {
        ...
        collectionView.snp.makeConstraints { make in
            make.edges.equalTo(UIEdgeInsets(top:  Screen.DFSafeAreaTopHeight() + 54, left: 0, bottom: 0, right: 0))
        }
        ...
        createButton.snp.makeConstraints { make in
            make.centerX.equalToSuperview()
            make.bottom.equalToSuperview().offset(-max(Screen.DFSafeAreaBottomHeight(), 10))
