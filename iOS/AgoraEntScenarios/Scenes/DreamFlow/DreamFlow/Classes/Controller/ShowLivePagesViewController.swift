...
class ShowLivePagesViewController: UIViewController {
    ...
    private lazy var delegateHandler = {
        ...
        handler.onRequireRenderVideo = {[weak self] info, canvas, cell, indexPath in
            ...
            DreamFlowLogger.info("[\(room.roomId)]onRequireRenderVideo: \(info.channelName)  \(vc.liveView.canvasView.localView)", context: kPagesVCTag)
            if room.channelName() == info.channelName, room.userId() == "\(info.uid)" {
                ...
                DreamFlowLogger.info("onRequireRenderVideo fail: \(info.channelName)/\(room.roomId)", context: kPagesVCTag)
                return nil
            }
        }
        return handler
    ...
    deinit {
        DreamFlowLogger.info("deinit-- ShowLivePagesViewController", context: kPagesVCTag)
    ...
    private func addDebugButton(){
        ...
        superbutton.setTitle("show_super_resolution".show_localized, for: .normal)
...
class ShowLivePagesSlicingDelegateHandler: AGCollectionSlicingDelegateHandler {
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        defer {
            DreamFlowLogger.info("collectionView cellForItemAt: \(idx)/\(indexPath.row)", context: kPagesVCTag)
        }
        ...
        vc.room = room
        //willDisplay will not be executed when count==1,
...
//            assert(false, "room at index \(idx) not found")
    ...
    override func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        ...
        if let cycleArray = roomList as? ShowCycleRoomArray {
            ...
            DreamFlowLogger.info("scrollViewDidEndDecelerating: from: \(currentIndex) to: \(fakeIndex)", context: kPagesVCTag)
            self.scrollView = nil
