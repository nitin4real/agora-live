...
//    }()
    ...
    func prepareEngine() {
        ...
        DreamFlowLogger.info("load AgoraRtcEngineKit, sdk version: \(AgoraRtcEngineKit.getSdkVersion())", context: kDreamFlowLogBaseContext)
    }
    
    func destoryEngine() {
        AgoraRtcEngineKit.destroy()
        ShowAgoraKitManager._sharedManager = nil
        DreamFlowLogger.info("deinit-- ShowAgoraKitManager")
    }
    // 退出已加入的频道和子频道
    func leaveAllRoom() {
    ...
    private func engineConfig() -> AgoraRtcEngineConfig {
        ...
        config.appId = DreamFlowContext.appId
    ...
    private func setupContentInspectConfig(_ enable: Bool, connection: AgoraRtcConnection) {
        ...
        let dic: [String: String] = [
            "id": DFUserCenter.user.id,
            "sceneName": "show",
            "userNo": DFUserCenter.user.userNo
        ]
        
        guard let jsonData = try? JSONSerialization.data(withJSONObject: dic, options: .prettyPrinted) else {
            DreamFlowLogger.error("setupContentInspectConfig fail")
            return
        }
...
//        DreamFlowLogger.info("setupContentInspectConfig: \(ret ?? -1)")
    }
    
    /// 语音审核
    private func moderationAudio(channelName: String, role: AgoraClientRole) {
        guard role == .broadcaster else { return }
        let userInfo = ["id": DFUserCenter.user.id,
                        "sceneName": "show",
                        "userNo": DFUserCenter.user.userNo,
                        "userName": DFUserCenter.user.name]
        let parasm: [String: Any] = ["appId": DreamFlowContext.appId,
        ...
        let baseURL = DreamFlowContext.releaseBaseServerUrl
        NetworkManager.shared.postRequest(urlString: "\(baseURL)/v1/moderation/audio",
                                          params: parasm) { response in
            DreamFlowLogger.info("response === \(response)")
        } failure: { errr in
            DreamFlowLogger.error(errr)
        }
    }
    ...
    private func _joinChannelEx(currentChannelId: String,
        ...
        let localUid = UInt(DFUserCenter.user.id)!
        if role == .audience {
        ...
        let date = Date()
        DreamFlowLogger.info("try to join room[\(connection.channelId)] ex uid: \(connection.localUid)", context: kDreamFlowLogBaseContext)
        let ret =
        engine.joinChannelEx(byToken: token,
            ...
            DreamFlowLogger.info("join room[\(channelName)] ex success uid: \(uid) cost \(cost) ms", context: kDreamFlowLogBaseContext)
...
//            self?.moderationAudio(channelName: targetChannelId, role: role)
        ...
        DreamFlowLogger.info("_joinChannelEx[\(connection.channelId)] role: \(mediaOptions.clientRoleType.rawValue) \(mediaOptions.publishMicrophoneTrack) \(mediaOptions.publishCameraTrack)")
        _updateChannelEx(options: mediaOptions, connection: connection)
        broadcasterConnection = connection

        if ret == 0 {
            DreamFlowLogger.info("join room ex: channelId: \(targetChannelId) ownerId: \(ownerId)",
                            context: "AgoraKitManager")
        }else{
            DreamFlowLogger.error("join room ex fail: channelId: \(targetChannelId) ownerId: \(ownerId) token = \(token), \(ret)",
                             context: kDreamFlowLogBaseContext)
        }
    }
    
    private func _updateChannelEx(options: AgoraRtcChannelMediaOptions,
                                  connection: AgoraRtcConnection) {
        DreamFlowLogger.info("_updateChannelEx[\(connection.channelId)][\(connection.localUid)] role: \(options.clientRoleType.rawValue) publishMicrophoneTrack:\(options.publishMicrophoneTrack) publishCameraTrack:\(options.publishCameraTrack)")
    ...
    func updateVideoEncoderConfigurationForConnenction(currentChannelId: String) {
        ...
        connection.localUid = UInt(DFUserCenter.user.id) ?? 0
        ...
        DreamFlowLogger.info("setVideoEncoderConfigurationEx  dimensions = \(encoderConfig.dimensions), bitrate = \(encoderConfig.bitrate), fps = \(encoderConfig.frameRate),  encoderRet = \(encoderRet)", context: kDreamFlowLogBaseContext)
    }
    
    //MARK: public method
    func addRtcDelegate(delegate: AgoraRtcEngineDelegate, roomId: String) {
        DreamFlowLogger.info("addRtcDelegate[\(roomId)]")
        ...
        if let _ = VideoLoaderApiImpl.shared.getConnectionMap()[roomId] {
            ...
            let localUid = Int(DFUserCenter.user.id)!
    ...
    func removeRtcDelegate(delegate: AgoraRtcEngineDelegate, roomId: String) {
        DreamFlowLogger.info("removeRtcDelegate[\(roomId)]")
        let localUid = Int(DFUserCenter.user.id)!
        let connection = AgoraRtcConnection(channelId: roomId, localUid: localUid)
    ...
    func renewToken(channelId: String) {
        DreamFlowLogger.info("renewToken with channelId: \(channelId)",
                  context: kDreamFlowLogBaseContext)
        preGenerateToken {[weak self] token in
            guard let token = token else {
                DreamFlowLogger.error("renewToken fail: token is empty")
                return
            }
    ...
    func enableVirtualBackground(isOn: Bool, greenCapacity: Float = 0) {
        ...
        DreamFlowLogger.info("isOn = \(isOn), enableVirtualBackground ret = \(ret)")
    }
    ...
    func updateChannelEx(channelId: String, options: AgoraRtcChannelMediaOptions) {
        guard let connection = (broadcasterConnection?.channelId == channelId ? broadcasterConnection : nil) ?? VideoLoaderApiImpl.shared.getConnectionMap()[channelId] else {
            DreamFlowLogger.error("updateChannelEx fail: connection is empty")
            return
        }
        
        DreamFlowLogger.info("updateChannelEx[\(channelId)] role: \(options.clientRoleType.rawValue) publishMicrophoneTrack:\(options.publishMicrophoneTrack) publishCameraTrack:\(options.publishCameraTrack)")
    ...
    func switchRole(role: AgoraClientRole? = nil,
        ...
        guard let uid = UInt(uid ?? "") else {
            DreamFlowLogger.error("switchRole fatel")
            return
        }
        
        DreamFlowLogger.info("switchRole[\(channelId)], role: \(role?.rawValue ?? -1) localUid:\(UserInfo.userId) uid: \(uid)", context: kDreamFlowLogBaseContext)
        if let role = role {
    ...
    func updateLiveView(role: AgoraClientRole? = nil,
                        ...
                        canvasView: UIView?) {
        guard let uid = UInt(uid ?? "") else {
            DreamFlowLogger.error("updateLiveView fatel")
            return
        }
    ...
    func preSubscribePKVideo(isOn: Bool, channelId: String) {
        ...
        let uid = Int(DFUserCenter.user.id) ?? 0
...
//        setupContentInspectConfig(false)
    ...
    func joinChannelEx(currentChannelId: String,
        ...
        _joinChannelEx(currentChannelId: currentChannelId,
                       ...
                       ownerId: ownerId,
                       token: DreamFlowContext.shared.rtcToken ?? "",
                       options: options,
                       role: role)
        completion?()
    }
    ...
    func setupLocalVideo(mirrorMode: AgoraVideoMirrorMode = .disabled,
        ...
        DreamFlowLogger.info("setupLocalVideo, user uid:\(UserInfo.userId) view: \(canvasView?.description ?? "")", context: kDreamFlowLogBaseContext)
    }
    
    private func setupRemoteVideo(channelId: String, uid: UInt, canvasView: UIView?) {
        let connection = AgoraRtcConnection(channelId: channelId, localUid: Int(DFUserCenter.user.id) ?? 0)
        //先清理老的view
        ...
        DreamFlowLogger.info("setupRemoteVideoEx[\(channelId)] ret = \(ret ?? -1), uid:\(uid) view: \(canvasView?.description ?? "")", context: kDreamFlowLogBaseContext)
    }
    ...
    func cleanChannel(without roomIds: [String]) {
        ...
        for (key, _) in videoLoader.getConnectionMap() {
            ...
            anchorInfo.uid = UInt(DFUserCenter.user.id)!
...
extension ShowAgoraKitManager {
    func getAnchorInfo(channelId: String, uid: UInt? = nil)->AnchorInfo {
        ...
        anchorInfo.uid = uid ?? (UInt(DFUserCenter.user.id) ?? 0)
        anchorInfo.token = DreamFlowContext.shared.rtcToken ?? ""
    ...
    func setOffMediaOptionsVideo(roomid: String) {
        guard let connection = VideoLoaderApiImpl.shared.getConnectionMap()[roomid] else {
            DreamFlowLogger.info("setOffMediaOptionsVideo  connection 不存在 \(roomid)")
            return
        }
        DreamFlowLogger.info("setOffMediaOptionsVideo with roomid = \(roomid)")
        let mediaOptions = AgoraRtcChannelMediaOptions()
...
extension ShowAgoraKitManager: IVideoLoaderApiListener {
    public func debugInfo(_ message: String) {
        DreamFlowLogger.info(message, context: "VideoLoaderApi")
    }
    public func debugWarning(_ message: String) {
        DreamFlowLogger.warn(message, context: "VideoLoaderApi")
    }
    public func debugError(_ message: String) {
        DreamFlowLogger.error(message, context: "VideoLoaderApi")
    }
}
...
extension ShowAgoraKitManager {
    func preGenerateToken(completion: ((String?)->())?) {
        DreamFlowContext.shared.rtcToken = nil
        NetworkManager.shared.generateToken(channelName: "",
            ...
            DreamFlowContext.shared.rtcToken = token
            completion?(token)
        }
    }
}
